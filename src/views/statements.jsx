...
const statements = {
    VariableDeclaration: ({expression = false, ...props}) => {
        ...
        const declarations = context.child("declarations").elements().map(renderDeclaration);
        const declarationsList = utils.commaSeparated(declarations);
        const tag = expression ? "span" : "div";
        return React.createElement(tag, {className: (expression ? "expression" : "statement") + " variable-declaration"},
            <span className="keyword" style={{width: "5ex"}}>var </span>,
            <span className="declarations">{declarationsList}{expression ? "" : ";"}</span>);
    },
    ...
    IfStatement: (props) => {
        ...
        const renderElseClause = (context, depth = 1) => {
            if (context.isEmpty()) {
                ...
                const result = [" ", <span className="keyword">else</span>, " "];
                if (context.node.get("type") === "IfStatement") {
                    Array.prototype.push.apply(result, renderIfClause(context, depth + 1));
                } else {
        ...
        const renderIfClause = (context, depth = 1) => {
            ...
            const consequent = _renderDepth(context.child("consequent"), dispatchStatement, depth);
            const result = [<span className="keyword">if</span>, " ", openParen, test, closeParen, " ", consequent];
            Array.prototype.push.apply(result, renderElseClause(context.child("alternate"), depth));
            return result;
        };
    ...
    SwitchStatement: (props) => {
        ...
        const caseClauses = context.child("cases").elements().map(renderCase).toArray();
        return (
    ...
    TryStatement: (props) => {
        ...
        const renderFinallyClause = (context) => {
            ...
            return (<span className="finally-clause"> <span className="keyword">finally</span> {finalizerRendered}</span>);
        };
        
        const block = context.child("block").render(dispatchStatement);
        const catchClause = renderCatchClause(context.child("handler"));
        const guardedCatchClauses = context.child("guardedHandlers").elements().map(renderCatchClause);
        return (
            <div className="statement try-statement">
                <span className="statement-header">
                    <span className="keyword">try</span>
                </span> {block} {catchClause} {guardedCatchClauses} {renderFinallyClause(context)}
            </div>
        );
    },
...
const UnknownStatement = (props) => {
    ...
    return (
        <div className="statement unknown-statement">
            <span className="inner" onClick={inspect}>{"<<"} unknown: {e} {">>"}</span>
        </div>
    );
};
